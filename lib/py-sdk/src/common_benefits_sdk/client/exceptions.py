...
class APIError(Exception):
    ...
    timeout) where no response was received. ``path`` is the request path (when known), and
    ``body`` carries the raw response text when available.
    """
    ...
    def __init__(
        ...
        self.body = body
        self.path = path
        super().__init__(f"{status}: {message}" + (f" ({path})" if path else ""))
